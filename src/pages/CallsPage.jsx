...
const AudioSeekBar = ({ currentTime, duration, onSeek }) => {
  ...
  return (
    <div className="w-full space-y-1">
      <div 
        className="relative h-1.5 bg-rose-500/20 rounded-full cursor-pointer group"
        onClick={handleSeek}
...
const CallCard = ({ call, index, isPlaying, onPlay, onPause, onEdit, onDelete }) => {
  ...
  if (isEditing) {
    return (
      <motion.div
        ...
        className="bg-gradient-to-br from-rose-950/20 to-black/40 backdrop-blur-md rounded-xl p-4 border border-rose-500/20"
      >
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="group relative bg-gradient-to-br from-rose-950/20 to-black/40 backdrop-blur-md rounded-xl p-4 border border-rose-500/20 hover:border-rose-500/40 transition-all duration-300"
    >
      <div className="absolute inset-0 bg-gradient-to-r from-rose-500/5 to-transparent rounded-xl opacity-0 group-hover:opacity-100 transition duration-500" />
      
      <div className="relative z-10">
        {/* Header with title and buttons */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 rounded-full bg-gradient-to-r from-rose-500 to-rose-600 flex items-center justify-center shadow-md">
              <Phone className="w-4 h-4 text-white" />
            </div>
            <p className="text-white font-bold text-lg">{call.title}</p>
            {isPlaying && (
          ...
          <div className="flex gap-1">
            <button 
              ...
              className="text-blue-400 hover:text-blue-300 transition p-1.5 rounded-lg hover:bg-white/10"
              title="Edit"
            >
        ...
        <div className="grid grid-cols-3 gap-2 mb-3 text-xs">
          <div className="flex items-center gap-1 text-rose-400">
            ...
            <span className="text-gray-300 text-xs">{displayDate.split(',')[0]}</span>
          </div>
          <div className="flex items-center gap-1 text-rose-400">
          ...
          <div className="flex items-center gap-1 text-rose-400">
            ...
            <span className="text-gray-300 text-xs font-mono">{call.durationFormatted || formatDuration(call.duration)}</span>
          </div>
        </div>
        
        {audioError && (
          <div className="mb-2 text-red-400 text-xs bg-red-500/10 p-1.5 rounded-lg">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <motion.button
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
              isPlaying 
                ? 'bg-gradient-to-r from-rose-500 to-rose-600 shadow-md'
                : 'bg-gradient-to-r from-rose-500/80 to-rose-600/80 hover:shadow-md'
          ...
          {isValidAudioUrl && !audioError && (
            <div className="flex items-center gap-1">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={toggleMute}
                className="w-6 h-6 rounded-full bg-rose-500/20 hover:bg-rose-500/40 flex items-center justify-center transition"
              >
                {isMuted ? <VolumeX className="w-3 h-3 text-rose-400" /> : <Volume2 className="w-3 h-3 text-rose-400" />}
              </motion.button>
              ...
              <input
                ...
                className="w-12 h-1 bg-rose-500/20 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-2 [&::-webkit-slider-thumb]:h-2 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-rose-400"
              />
            </div>
          )}
        </div>
...
export default function CallsPage() {
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="relative z-10 pb-20">
        ...
        <div className="sticky top-0 z-20 bg-black/80 backdrop-blur-md border-b border-rose-500/20">
          <div className="container mx-auto px-4 py-3">
            <button
              onClick={() => navigate('/')}
              className="inline-flex items-center gap-1 text-rose-400 hover:text-rose-300 transition mb-2 text-sm"
            >
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
              <div>
                <h1 className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-rose-400 to-rose-600 bg-clip-text text-transparent">
                  Call Memories
                </h1>
                <p className="text-gray-400 text-xs mt-0.5">82 Special Voice Calls 💕</p>
              </div>
              
              <div className="flex gap-2">
                <div className="bg-black/50 backdrop-blur-sm rounded-lg px-3 py-1 border border-rose-500/20">
                  <p className="text-rose-400 text-[10px]">Total Calls</p>
                  <p className="text-white text-lg font-bold">{totalCalls}</p>
                </div>
                <div className="bg-black/50 backdrop-blur-sm rounded-lg px-3 py-1 border border-rose-500/20">
                  <p className="text-rose-400 text-[10px]">Total Duration</p>
                  <p className="text-white text-lg font-bold">{Math.floor(totalDuration / 3600)}h {Math.floor((totalDuration % 3600) / 60)}m</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mb-4 flex flex-col sm:flex-row gap-3 justify-between items-center">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full bg-black/50 border border-rose-500/30 rounded-lg pl-9 pr-3 py-1.5 text-white placeholder-gray-500 text-sm focus:outline-none focus:border-rose-500 transition"
              />
            </div>
            <button
              onClick={() => setShowAddModal(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-rose-500 to-rose-600 text-white hover:shadow-md transition text-sm"
            >
          ...
          {filteredCalls.length === 0 ? (
            ...
            >
              <div className="relative">
                <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-rose-500/20 to-amber-500/20 rounded-full flex items-center justify-center">
                  <Phone className="w-10 h-10 text-rose-400 opacity-50" />
                </div>
              </div>
      ...
      <button
        onClick={() => setShowAddModal(true)}
        className="fixed bottom-5 right-5 z-20 bg-gradient-to-r from-rose-500 to-rose-600 p-3 rounded-full shadow-lg hover:shadow-xl transition-all group"
      >
        <Plus className="w-5 h-5 text-white group-hover:scale-110 transition" />
      </button>
